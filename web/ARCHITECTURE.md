...
- Analysed previews use the detector's [latest-frame transport](../detector/LIVE_PREVIEW.md), without opening another camera connection. The server validates records once and owns viewer leases/SSE cleanup. The UI shows matching image/boxes, tracks connection freshness, and releases invisible previews. `PreviewSlots` caps active feeds at four per browser tab. Live views never establish recording or alert success.
- Batch setup coordinates bounded ONVIF connection attempts and keeps successful results in memory. Each selected camera still passes the existing recording check, explicit confirmation, configuration save and completion checks. It adds no second persistence path or batch bypass around verification.
...
Preset definitions are data in `config/presets.json` and its referenced detector fragments. `configuration/preset-catalog.ts` validates catalogue structure and detector options against the canonical schema; `configuration/presets.ts` selects the runtime catalogue or the files embedded by Vite. A `presets.json` in the data folder, or an explicit `AIDETECTOR_PRESETS` path, supplies installation-specific choices without rebuilding. `editor-schema.ts` independently handles advanced editor schema lookup.

`ConfigurationStore` receives the catalogue loader explicitly. Only choosing a new preset loads its definition. Camera actions use a separate `mode` (`preset`, `keep`, `copy`, `view-only`); preset IDs are opaque strings, including IDs that happen to match an action name. A saved detector owns its copied configuration, not a live reference to the catalogue. Existing IDs and actual rule settings survive catalogue edits or removal. UI names, descriptions and guidance come from catalogue metadata; there are no use-case-specific branches in the application.
