...
def package(detector: Path, web: Path, ffmpeg: Path, output: Path, platform: str, image: str | None) -> Path:
    output.mkdir(parents=True, exist_ok=True)
    name = f"AI-Detector-{platform}"
    folder = output / name
    folder.mkdir()
    shutil.copytree(detector, folder / "detector")
    launch = "AI Detector.exe" if platform == "windows-x64" else "AI Detector.command" if platform.startswith("macos") else "AI Detector"
    shutil.copy2(web, folder / launch)
    (folder / launch).chmod(0o755)
    (folder / "bin").mkdir()
    shutil.copy2(ffmpeg, folder / "bin" / ("ffmpeg.exe" if platform == "windows-x64" else "ffmpeg"))
    (folder / "application.json").write_text(json.dumps({"dockerImage": image} if image else {}) + "\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image", help="Matching image digest; omit for a native-only local preview")
