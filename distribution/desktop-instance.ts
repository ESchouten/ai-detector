...
export class DesktopInstance {
	...
	async existing(port: number, quit = false, timeout = 5000): Promise<boolean> {
		...
		do {
			let saved: { port: number; token: string; pid: number } | undefined;
			try {
				saved = JSON.parse(readFileSync(this.file, 'utf8'));
				if (!saved || saved.port !== port || typeof saved.token !== 'string') return false;
				const challenge = randomBytes(32).toString('hex');
				const response = await fetch(`http://127.0.0.1:${port}${CONTROL_PATH}`, {
				...
				if (response.ok) {
					...
					if (
						...
						return quit ? waitForExit(saved.pid, 45000) : true;
				}
			} catch (error) {
