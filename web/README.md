...
**Set up several cameras** discovers and connects cameras sharing a login, with two connection attempts at a time. Each queued camera uses the same picture/test-recording confirmation and save flow as a single camera. Failed connections can be retried without losing successes. The queue and its credentials stay in memory; keep that tab open. Finish-setup links open separately so the remaining queue is preserved.

**Show detections** on a camera displays the detector's analysed frame, boxes, confidence and optional temporary track IDs. Cameras with multiple rules offer a rule selector. **Show live picture** remains a separate direct camera view. Hidden or offscreen previews release their connections; four previews can run in one browser tab. Detector settings expose optional overlap (IoU) and tracker choices under **Tracking and overlapping detections**; empty choices preserve Ultralytics defaults.
